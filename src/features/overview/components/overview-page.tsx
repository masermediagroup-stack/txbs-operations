...
export function OverviewPage({ role = "Operator" }: { role?: "Operator" | "Tech" }) {
  ...
  return <div className="flex flex-1 flex-col gap-6">
    ...
    <section aria-labelledby="overview-kpis"><h2 id="overview-kpis" className="sr-only">Current inventory indicators</h2><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Projects present" value={metrics.projects} detail="Projects with material on site" href="/inventory/projects" icon={Boxes} /><MetricCard label="Present lots" value={metrics.presentLots} detail="Physical material records" href="/inventory/materials" icon={PackageCheck} /><MetricCard label="Verification due" value={metrics.verificationDue} detail="Overdue or never verified" href="/reports" icon={ClipboardCheck} /><MetricCard label="Blocking issues" value={metrics.blockingIssues} detail="Active readiness blockers" href="/inventory/issues" icon={ShieldAlert} /></div></section>
    ...
    <div className="grid gap-6 xl:grid-cols-2">
      <Card><CardHeader><CardTitle>Attention</CardTitle><CardDescription>Open work that can affect material readiness.</CardDescription></CardHeader><CardContent className="grid gap-2">{[
        { label: "Verification due", value: metrics.attention.verificationDue, href: "/reports", icon: ClipboardCheck },
    ...
    <Card><CardHeader><CardTitle>Quick actions</CardTitle><CardDescription>Start the workflows used most often during material operations.</CardDescription></CardHeader><CardContent className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><Button nativeButton={false} render={<Link href="/inventory/receiving" />} variant="outline"><ClipboardCheck aria-hidden="true" data-icon="inline-start" />Receive material</Button><Button nativeButton={false} render={<Link href="/inventory/movements" />} variant="outline"><ArrowRightLeft aria-hidden="true" data-icon="inline-start" />Move material</Button><Button nativeButton={false} render={<Link href="/reports" />} variant="outline"><Search aria-hidden="true" data-icon="inline-start" />Verify material</Button><Button nativeButton={false} render={<Link href="/inventory/outbound" />} variant="outline"><PackageCheck aria-hidden="true" data-icon="inline-start" />Prepare outbound</Button></CardContent></Card>
