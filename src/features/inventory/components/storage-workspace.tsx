...
export function StorageWorkspace({ slug }: { slug: string }) {
  ...
  if (!location) return <div className="rounded-xl border border-dashed p-8 text-center">Storage location not available.</div>
  ...
  return <div className="flex flex-1 flex-col gap-6">
    <PageHeader eyebrow={`${location.type} storage`} title={location.name} description={`${location.zone} · Lavon Yard`} />
