...
export function ProjectWorkspace({ slug }: { slug: string }) {
  ...
  return <div className="flex flex-1 flex-col gap-6">
      ...
      <CardHeader className="border-b">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            {readiness.verificationDue.length ? <Link href="#material-verification" className={buttonVariants({ size: "lg" })}><ClipboardCheck aria-hidden="true" data-icon="inline-start" />Verify {readiness.verificationDue.length} material{readiness.verificationDue.length === 1 ? "" : "s"}</Link> : null}
            <Link href={`/inventory/outbound?project=${project.id}`} className={buttonVariants({ size: "lg", variant: readiness.verificationDue.length ? "outline" : "default" })}><PackageOpen aria-hidden="true" data-icon="inline-start" />Prepare outbound</Link>
          </div>
        </div>
      </CardHeader>
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.6fr)_minmax(18rem,0.7fr)]">
      <Card id="material-verification" className="scroll-mt-24"><CardHeader className="relative border-b"><span aria-hidden="true" className="absolute inset-x-0 -bottom-px h-1 bg-brand-orange" /><CardTitle>Material verification</CardTitle><CardDescription>Confirm each material lot where it is physically stored. The saved location is prefilled, and photos are optional.</CardDescription></CardHeader><CardContent>{lots.length ? <LotLedger lots={lots} snapshot={snapshot} /> : <p className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">No material lots are present for this project.</p>}</CardContent></Card>
