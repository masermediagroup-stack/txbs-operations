...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("execute_inventory_command_v1", {
    ...
    if (error) {
      const status = error.code === "40001" || error.code === "23505"
        ? 409
        : error.code === "P0002"
          ? 404
          : error.code === "42501"
    ...
    if (error instanceof ZodError) {
      return NextResponse.json(
        { error: "The command was not valid.", issues: error.issues },
        { status: 400 },
      )
    }
    console.error("Inventory command failed", error)
    return NextResponse.json({ error: "The shared command could not be completed." }, { status: 500 })
  }
}
