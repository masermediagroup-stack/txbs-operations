...
const evidencePhotos = [
  "public/images/storage/conex-container.png",
  "public/images/storage/yard-area.png",
  "public/images/storage/conex-container-white.png",
];
...
test("known-project receiving creates a durable material lot", async ({ page }, testInfo) => {
  ...
  for (const number of [1, 2, 3]) {
    await page
      .getByLabel(`Add packing slip / document photos ${number} of 3`)
      .setInputFiles(evidencePhotos[number - 1]);
  }
  await expect(
    page.getByRole("button", {
      name: /Remove packing slip \/ document photos/,
    }),
  ).toHaveCount(3);
  await page.screenshot({
    path: testInfo.outputPath("receiving-photo-slots.png"),
    fullPage: true,
  });
  ...
  await page
    .getByLabel("Storage assignment")
    .selectOption({ label: "Conex 4" });
  for (const number of [1, 2, 3]) {
    await page
      .getByLabel(`Add material photos ${number} of 3`)
      .setInputFiles(evidencePhotos[number - 1]);
  }
  await page.getByLabel("Physical handwritten project label applied").check();
  await page
    .getByLabel("Staging / default location")
    .selectOption({ label: "Conex 4" });
  ...
  await expect(
    page.getByRole("heading", { name: "E2E field accessories" }),
  ).toBeVisible();
});

test("receiving requires line photos while allowing a photo-free draft", async ({
  page,
}) => {
  await page.goto("/inventory/receiving");
  await page
  ...
  await page.getByRole("button", { name: "Review and receive" }).click();
  await expect(
    page.getByText("Add a material photo to receipt line 1 before receiving."),
  ).toBeVisible();
